...
@DesignerComponent(version = YaVersion.WEBVIEWER_COMPONENT_VERSION,
    ...
    description = "Entity Grid is a custom-made easy-to-use table control to present "
        + "entities extracted from Data.Esquel component.")

// TODO(halabelson): Integrate control of the Back key, when we provide it

@SimpleObject
@UsesPermissions(permissionNames = "android.permission.INTERNET")
public final class EntityGrid extends AndroidViewComponent {
   ...
   */
  public EntityGrid(ComponentContainer container) {
    super(container);
...
//  }
   ...
   *
   * @param columnIndex, the index of the column to be mapped. Index:={1>n} from left to right
   * @param columnName, the name of the column in the table header row
   * @param field, the entity field mapping to the column
   */
  @SimpleFunction(description = "Mapping the Entity field to a specific column: <br> ColumnName is the name of the column in the table header. Field is the entity field mappign to the column.")
  public void MapEntityField(final String columnName, final String entityFieldName) {
    titleList.addLast(columnName);
    fieldList.addLast(entityFieldName);
    return;
  }
  
  /**
   * Refresh data to the Entity Grid in the interface
   ...
   * @param entitySet
   */
  @SimpleFunction(description = "Refresh the data from Entity to the table.")
  public void Refresh(final Object entity) {
    EsqEntityList entityObj = (EsqEntityList) entity;
    String resultHTML = "";
    webview.getSettings().setDefaultTextEncodingName("UTF-8");
   ...
   //Generate the parsing script using AngularJS
    String jscriptPart = "var app = angular.module('myApp', []); function EntityCtrl($scope, $http) { $scope.entityJson = JSON.parse('"+ entityObj.getJsonText() + "');}";
    
    //Generate the html table with AngularJS binding
    String tablePart = "<div ng-app=\"myApp\"> <div ng-controller=\"EntityCtrl\" class=\"datagrid\"><table> <thead><tr>";
     Iterator<String> iterator = titleList.iterator();
     while (iterator.hasNext()) {
       tablePart += "<th>" + iterator.next() + "</th>";
     }
     
     tablePart +="</tr> </thead> <tbody><tr ng-repeat=\"entity in entityJson.DailySalesPerStore\">";
     ...
     //webview.loadDataWithBaseURL("blarg://ignored", resultHTML, "text/html", "utf-8", null);
